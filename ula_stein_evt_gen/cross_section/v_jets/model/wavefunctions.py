...
from model.madjax_patch import sqrt, complex, max, min
...
def _massless_in_chi(p, nhel, nsf, nh):
    beam_along_negative_z = np.logical_and(
        np.logical_and(p[1] == 0.0, p[2] == 0.0),
        p[3] < 0.0,
    )
    safe_p0p3 = np.where(beam_along_negative_z, 1.0, max(p[0] + p[3], 0.0))
    sqp0p3 = sqrt(safe_p0p3) * nsf
    beam_chi = np.array([
        complex(0.0, 0.0),
        complex(-1.0 * nhel * sqrt(2.0 * p[0]), 0.0),
    ])
    generic_chi = np.array([
        complex(sqp0p3, 0.0),
        complex(nh * p[1] / sqp0p3, p[2] / sqp0p3),
    ])
    return np.where(beam_along_negative_z, beam_chi, generic_chi)

def _massless_out_chi(p, nhel, nsf, nh):
    beam_along_negative_z = np.logical_and(
        np.logical_and(p[1] == 0.0, p[2] == 0.0),
        p[3] < 0.0,
    )
...
def _massive_in_chi(p, nh, pp, pp3):
    beam_along_negative_z = pp3 == 0.0
    safe_pp = np.where(beam_along_negative_z, 1.0, pp)
    safe_pp3 = np.where(beam_along_negative_z, 1.0, pp3)
    denom = sqrt(2.0 * safe_pp * safe_pp3)
    beam_chi = np.array([
        complex(0.0, 0.0),
        complex(-1.0 * nh, 0.0),
    ])
    generic_chi = np.array([
        complex(sqrt(safe_pp3 * 0.5 / safe_pp), 0.0),
        complex(nh * p[1] / denom, p[2] / denom),
    ])
    return np.where(beam_along_negative_z, beam_chi, generic_chi)

def _massive_out_chi(p, nh, pp, pp3):
    beam_along_negative_z = pp3 == 0.0
    safe_pp = np.where(beam_along_negative_z, 1.0, pp)
    safe_pp3 = np.where(beam_along_negative_z, 1.0, pp3)
    denom = sqrt(2.0 * safe_pp * safe_pp3)
...
def ixxxxx(p, fmass, nhel, nsf):
    ...
    def fmass_nonzero_pp_nonzero(p, fmass, nhel, nsf, nh, pp):
        ...
        pp3 = max(pp + p[3], 0.0)
        chi = _massive_in_chi(p, nh, pp, pp3)
        chi0 = chi[0]
        chi1 = chi[1]
    ...
    def fmass_zero(p, fmass, nhel, nsf, nh):
        chi = _massless_in_chi(p, nhel, nsf, nh)
...
def ixxxxx_massless(p, fmass, nhel, nsf):
    ...
    fi2345 =fmass_zero(p, nhel, nsf, nh)
...
def ixxxxx_massive(p, fmass, nhel, nsf):
    ...
    fi2345 = fmass_nonzero(p, fmass, nhel, nsf, nh)
...
def oxxxxx(p, fmass, nhel, nsf):
    ...
    def fmass_nonzero_pp_nonzero(p, fmass, nhel, nsf, nh, pp):
        ...
        pp3 = max(pp + p[3], 0.0)
        chi = _massive_out_chi(p, nh, pp, pp3)
        chi0 = chi[0]
        chi1 = chi[1]
    ...
    def fmass_zero(p, fmass, nhel, nsf, nh):
        chi = _massless_out_chi(p, nhel, nsf, nh)
...
def oxxxxx_massless(p, fmass, nhel, nsf):
    ...
    fo2345 = fmass_zero(p, nhel, nsf, nh)
...
def oxxxxx_massive(p, fmass, nhel, nsf):
    ...
    fo2345 = fmass_nonzero(p, fmass, nhel, nsf, nh)
...
def sign(x, y):
    _sign = lax.cond(np.issubdtype(y.dtype, np.complexfloating), y.real, np.sign, y, np.sign)
